...
def load_mnist_trainval():
    ...
    # train_idx = random.sample(val_idx,train_n)
    #
    # train_data = []
    # train_label = []
    ...
    train_label = label[:train_n]

    val_data = data[train_n:]
    val_label = label[train_n:]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    batched_data = [np.array(data_shuffle[i:i+batch_size]) for i in range(0,n,batch_size)]
    batched_label = [np.array(label_shuffle[i:i+batch_size]) for i in range(0,n,batch_size)]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    fig.suptitle('left: Loss curve, right: Accuracy curve')
    ax1.plot(train_loss_history,label='train')
    ax1.plot(valid_loss_history,label='valid')
    ax1.set_xlabel('Epochs')
    ax1.set_ylabel('Loss')
    ax1.legend()
    ax2.plot(train_acc_history, label = 'train')
    ax2.plot(valid_acc_history, label = 'valid')
    ax1.set_xlabel('Epochs')
