...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        #       outputs                                                             #
        #############################################################################
        first_out = np.dot(X,self.weights['W1']+self.weights['b1'])
        sigmoid_out = self.sigmoid(first_out)
        second_out = np.dot(sigmoid_out,self.weights['W2'])+self.weights['b2']
        softmax_out = self.softmax(second_out)
        loss = self.cross_entropy_loss(softmax_out,y)
        accuracy = self.compute_accuracy(softmax_out,y)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        N = y.shape[0]
        one_hot_y = np.zeros(softmax_out.shape)
        one_hot_y[range(N), y] = 1.0
        out_dev = softmax_out - one_hot_y
        self.gradients['b2'] = np.sum(out_dev,axis=0)/N
        self.gradients['W2'] = np.dot(sigmoid_out.T, out_dev)/N

        sigmoid_out_dev = self.sigmoid_dev(first_out)*np.dot(out_dev,self.weights['W2'].T)
        self.gradients['b1'] = np.sum(sigmoid_out_dev,axis=0)/N
        self.gradients['W1'] = np.dot(X.T, sigmoid_out_dev)/N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
